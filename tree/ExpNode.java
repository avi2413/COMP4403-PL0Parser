...
public abstract class ExpNode {
    ...
    public static class FormalParamNode extends ExpNode {
        private final SymEntry.ParamEntry param;
        private final ExpNode lVal;

        public FormalParamNode(Location loc, SymEntry.ParamEntry param) {
            super(loc);
            this.param = param;
            this.lVal = null;
        }

        public FormalParamNode(Location loc, SymEntry.ParamEntry param, ExpNode lVal) {
            super(loc);
            this.param = param;
            this.lVal = lVal;
        }

        public SymEntry.ParamEntry getParameter() {
            return param;
        }

        @Override
        public ExpNode transform(ExpTransform<ExpNode> visitor) {
            return visitor.visitFormalParamNode(this);
        }
        ...
        public String toString() {
            if (param.getDefaultExp() != null) {
               return param.getIdent() + ":" + param.getType() + "<-" + param.getDefaultExp();
            }
            return  "var" + param.getIdent() + ":" + param.getType() + "<-" + lVal;
        }
    }
